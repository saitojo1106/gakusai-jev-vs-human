...
日本語と英語。`?lang=ja` / `?lang=en` で切り替わり、選んだ言語は Cookie（1 年）に残る。
明示指定がなければ `Accept-Language` を見て、判定できなければ日本語。

文言は 2 箇所に分かれている。乗客の調書・所見・称号といった**内容**は
[i18n.ts](packages/domain/src/i18n.ts) の `Localized`（`{ ja, en }`）として domain 側に、
ボタンや見出しといった**画面の文言**は [i18n.ts](apps/site/app/i18n.ts) の `Ui` に置く。

乗客は毎回シードから再生成するので、言語を変えても同じ乗客のまま文面だけ入れ替わる。
乱数は言語に依存しない（`Localized` を引いてから最後に `[locale]` で解決する）。
Jev に渡す調書だけは常に日本語で作る。表示言語で Jev の判定が変わらないようにするため。
